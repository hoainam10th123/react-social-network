...
export default class PresenceHubStore {
    ...
    createHubConnection = (user: IUser) => {
        ...
        this.hubConnection.on('DisplayInformationCaller', (userCalling: IMember) => {
            runInAction(() =>{
                this.usernameCalling = userCalling.userName;
                callerMessageService.sendMessage(userCalling);
            })            
        })
    ...
    callToOtherUsername = async (otherUsername: string)=>{
        try {
            await this.hubConnection?.invoke('CallToUsername',  otherUsername);
        } catch (error) {
            console.log(error);
        }  
    }
